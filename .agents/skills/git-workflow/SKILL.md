...
> **Lab overlay:** Do not run `git config --global` from this skill. ShadowSpark Agent OS Lab forbids global git configuration mutation. Project-local config only, and only when the operator asks.
...
Lab overlay: do **not** run `git config --global`. Identity and pull/rebase policy are operator-owned. If a project needs a template, use **local** `git config` (no `--global`) after the operator asks.
