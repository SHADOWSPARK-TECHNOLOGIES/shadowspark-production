...
import {
  validateLoanTransition,
  isValidLoanTransition,
} from "@/lib/api/v1/state-machine";
...
beforeEach(() => {
  ...
  process.env.JWT_SECRET =
    process.env.JWT_SECRET ?? "test-secret-32-bytes-long-abc123";
});
...
describe("Auth", () => {
  ...
  it("hashes passwords with bcrypt", async () => {
    const hash = await bcrypt.hash("Demo@2026!", 4);
  ...
  it("getLoanById returns null when loan belongs to another tenant", async () => {
    ...
    expect(mockPrisma.loanApplication.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id: "loan_b", tenantId: "tenant_a" }),
      }),
    );
  });
});
...
describe("Idempotency", () => {
  it("returns 400 when Idempotency-Key header is missing", async () => {
    ...
    const response = await withIdempotency(request, "tenant_1", async () =>
  ...
  it("caches and replays the response scoped by tenant", async () => {
    ...
    const replayRequest = await makeJsonRequest(
      {},
    ...
    const otherTenantRequest = await makeJsonRequest(
      {},
      { "Idempotency-Key": "key-1" },
    );
...
describe("Loan Lifecycle State Machine", () => {
  ...
  it("throws on invalid transition", () => {
    expect(() => validateLoanTransition("SUBMITTED", "DISBURSED")).toThrow(
      /INVALID_TRANSITION/,
    );
  });
});
...
describe("KYC Pipeline", () => {
  it("advances loan to KYC_VERIFIED when all docs verified", async () => {
    ...
    expect(mockPrisma.loanApplication.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "KYC_VERIFIED" }),
      }),
    );
  ...
  it("keeps loan KYC_PENDING when one doc is rejected without auto-reject", async () => {
    ...
    await verifyKycDocument(
      "t1",
      "kyc_1",
      { status: "REJECTED", rejectionReason: "Blurry", autoRejectLoan: false },
      "user_1",
    );
...
describe("Messages", () => {
  it("creates message with status QUEUED and enqueues worker", async () => {
    ...
    await sendMessage(
      "t1",
      { loanApplicationId: "loan_1", channel: "sms", content: "Hello" },
      "user_1",
    );
...
describe("Workflows", () => {
  it("executes a simple task workflow and logs audit", async () => {
    ...
    mockPrisma.workflow.findFirst.mockResolvedValueOnce({
      ...
      nodes: [
        { id: "start", type: "start" },
        {
          id: "task",
          type: "task",
          config: { assignVariable: "approved", value: true },
        },
        { id: "end", type: "end" },
      ],
      edges: [
    ...
    expect(mockPrisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ action: "WORKFLOW_EXECUTED" }),
      }),
    );
  });
});
...
describe("Audit Logs", () => {
  it("creates audit log when API key is created", async () => {
    ...
    expect(mockPrisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ action: "API_KEY_CREATED" }),
      }),
    );
  });
...
describe("Secrets", () => {
  it("does not include raw API key in stored record", async () => {
    ...
    expect(mockPrisma.apiKey.create).not.toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ key: result.key }),
      }),
    );
  });
});
